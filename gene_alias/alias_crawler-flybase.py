...
aleady_parse = pd.read_csv(
    "./csv_data/flybase_gene-combine.csv", encoding="utf-8", header=None
)[0].to_list()
...
# crawler data
with open(
    "./csv_data/flybase_gene-part-1.csv", "w", newline="", encoding="utf-8"
) as csvfile:
    ...
    for fbid in gene_data.keys():
        ...
        aka_frag = soup.find(
            "div", string=re.compile("Also Known As")
        )  # check if aka exist
