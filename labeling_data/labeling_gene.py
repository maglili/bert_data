...
df = pd.read_csv(
    "../csv_data/output-pos-remove_no_abst.csv", header=None, encoding="utf-8"
)
df = df[[0, 2]]
...
with open(filename, "w", newline="", encoding="utf-8") as csvfile:
    ...
    for idx in range(len(df)):
        ...
        # Processing alias e.g. su(H), -> su(H)
        ...
        for i in range(len(save_range)):
            for j in range(i + 1, len(save_range)):
                if (save_range[i][0] >= save_range[j][0]) and (
                    save_range[i][1] <= save_range[j][1]
                ):
                    remove_list.append(save_range[i])
                if (save_range[j][0] >= save_range[i][0]) and (
                    save_range[j][1] <= save_range[i][1]
                ):
